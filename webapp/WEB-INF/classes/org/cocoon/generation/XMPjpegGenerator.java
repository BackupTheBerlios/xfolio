...
The XMPjpegGenerator extracts [XMP] from a jpeg file.
This AdobeTM XML can encode metadatas like title, author, description...
This schema implements an open standard knowed as [IPTC/IMM]
For your photos, try for example an app like [PixVue], 
it allows you to add different infos, 
encoded in jpeg headers, with also an XML/XMP embed.

This class extract the XML/XMP form a jpeg file for a 
...
[FG] Glorieux, Frédéric [AJLSM] <frederic.glorieux@xfolio.org> 
	Integration of W3C classes as a cocoon generator 
[PD] Dittgen, Pierre [PassTech] <pierre.dittgen@pass-tech.fr>
	Refactoring W3C classes
[nwalsh] Walsh, Norman <nwalsh@users.sourceforge.net>
	Seems the most active maintener of this classes
[PF] Fumagalli, Pierpaolo (Apache Software Foundation)
[CZ] Ziegeler, Carsten
	Original authors of [FileGenerator]
...
[XMP] Adobe Systems Incorporated "Extensible Metadata Platform (XMP)"  
	...
	Adobe's Extensible Metadata Platform (XMP) is a labeling technology 
	that allows you to embed data about a file, known as metadata, 
	into the file itself. With XMP, desktop applications and back-end 
	publishing systems gain a common method for capturing, sharing, 
	and leveraging this valuable metadata - opening the door for more 
	efficient job processing, workflow automation, and rights management, 
	among many other possibilities. With XMP, Adobe has taken the 
	"heavy lifting" out of metadata integration, offering content creators
	an easy way to embed meaningful information about their projects and 
	providing industry partners with standards-based building blocks to 
	develop optimized workflow solutions.
[PassTech] Boutros, Nader ; Dittgen, Pierre 
	<http://pass-tech.fr/>
[AJLSM] Arvers, Jean-Luc; Sévigny, Martin  
	"Solution pour le document numérique" 
	<http://ajlsm.com>
...
public class XMPjpegGenerator extends ServiceableGenerator
	...
	public void generate() throws IOException, SAXException,
		...
		// TODO, better Exception handling in case of guilty image
				...
				getLogger().error("XMP Generator " + this.inputSource.getURI() , e);
				xmp="<test/>";
			}
            parser.parse( new InputSource ( new StringReader(xmp)), this.contentHandler );
     ...
     * Gets XMP in APP1.
     *
     * from http://cvs.sourceforge.net/viewcvs.py/jpegrdf/jpegrdf/src/org/w3c/tools/jpeg/JpegHeaders.java
     *
     * @return XMP info as a unicode string
     * @throws IOException If a I/O read error occurred
     * @throws JpegException For all other reasons
     */
	public String getXMP() throws IOException, JpegException {
        ...
        // Then closing '>'
        endIndex = find(input, endIndex, '>') + 1;
        if (endIndex == 0)
            throw new JpegException("XMP not valid XML");
