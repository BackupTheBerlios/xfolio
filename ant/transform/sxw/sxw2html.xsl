...
Title     : Transform OpenOffice.org Writer XML in semantic XHTML
Label     : sxw2html.xsl
...
-->
    <img id="{@draw:name}" class="oo" alt="{svg:desc}"  align="{$align}" 
    width="{$size}" border="0">
      <!--
	If image is not in frame or table, a width attribute could be add
	width="{$width}%", otherwise, big layout problems could arrive.
  TODO : best is probably store % somewhere, and let script calculate pixel size
-->
      <xsl:choose>
